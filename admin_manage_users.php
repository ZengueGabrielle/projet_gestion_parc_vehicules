...
if (isset($_POST['add_user'])) {
    try {
        ...
        if ($stmt_check->rowCount() > 0) {
            ...
            $stmt = $pdo->prepare("INSERT INTO utilisateurs (nom, prenom, nom_utilisateur, email, mot_de_passe, role_id, created_at) VALUES (?, ?, ?, ?, ?, 3, NOW())");
            $stmt->execute([$nom, $prenom, $nom_utilisateur, $email, $mot_de_passe]);
            header('Location: admin_manage_users.php?success=Utilisateur ajouté');
            exit;
        }
...
if (isset($_POST['edit_user'])) {
    try {
        ...
        if ($stmt_check->rowCount() > 0) {
            ...
            header('Location: admin_manage_users.php?success=Utilisateur modifié');
            exit;
        }
    } catch (PDOException $e) {
...
if (isset($_POST['delete_user'])) {
    try {
        $id = $_POST['id'];
        $stmt = $pdo->prepare("DELETE FROM utilisateurs WHERE id = ? AND role_id = 3");
        $stmt->execute([$id]);
        header('Location: admin_manage_users.php?success=Utilisateur supprimé');
        exit;
    } catch (PDOException $e) {
                    ...
                    <table>
                        ...
                        <?php foreach ($utilisateurs as $u): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($u['nom_role']); ?></td>
                                <td><?php echo htmlspecialchars($u['created_at']); ?></td>
                                <td>
                                    <button class="btn btn-warning" onclick="openModal(
                                        <?php echo $u['id']; ?>,
                                        '<?php echo htmlspecialchars(addslashes($u['nom'])); ?>',
                                        '<?php echo htmlspecialchars(addslashes($u['prenom'])); ?>',
                                        '<?php echo htmlspecialchars(addslashes($u['nom_utilisateur'])); ?>',
