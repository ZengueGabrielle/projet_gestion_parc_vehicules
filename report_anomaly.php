...
$stmt = $pdo->prepare("SELECT v.id, v.marque, v.modele, v.immatriculation 
                      FROM vehicules v 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($vehicle_id && $commentaire) {
        $stmt = $pdo->prepare("INSERT INTO anomalies (utilisateur_id, vehicule_id, commentaire) VALUES (:user_id, :vehicle_id, :commentaire)");
        $stmt->execute([
...
<head>
    ...
    <style>
        ...
        .form-container { background: rgba(255, 255, 255, 0.9); padding: 30px; border-radius: 20px; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.3); max-width: 600px; margin: 20px auto; border: 3px solid #dc3545; color: #333; }
        h2 { color: #dc3545; font-size: 28px; margin-bottom: 20px; }
        form { display: flex; flex-direction: column; gap: 15px; }
        select, textarea { padding: 10px; border: 2px solid #dc3545; border-radius: 5px; font-size: 16px; }
        textarea { min-height: 100px; resize: vertical; }
        button { background: #dc3545; color: #fff; padding: 10px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; }
                    ...
                    <select name="vehicle_id" required>
                        ...
                        <?php foreach ($vehicles as $vehicle): ?>
                            <option value="<?php echo $vehicle['id']; ?>">
                                <?php echo htmlspecialchars($vehicle['marque'] . ' ' . $vehicle['modele'] . ' (' . $vehicle['immatriculation'] . ')'); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <textarea name="commentaire" placeholder="Décrivez l'anomalie" required></textarea>
