...
// Récupérer les maintenances prévues
$stmt = $pdo->prepare("
    SELECT pm.id, v.marque, v.modele, pm.type_maintenance, pm.date_prevue, pm.probabilite
    FROM predictions_maintenance pm
    JOIN vehicules v ON pm.vehicule_id = v.id
    WHERE pm.vehicule_id IN (
        SELECT vehicule_id FROM affectations WHERE utilisateur_id = ?
    )
    ORDER BY pm.date_prevue ASC
");
...
<head>
    ...
    <title>Maintenance Prévues</title>
...
<body>
    <video class="background-video" autoplay loop muted playsinline>
        ...
        Votre browser ne supporte pas la lecture de vidéos.
    </video>
    <header class="header">
                        ...
                        <tbody>
                            <?php foreach ($maintenances as $maintenance): ?>
                                <tr>
                                    ...
                                    <td><?php echo date('d/m/Y', strtotime($maintenance['date_prevue'])); ?></td>
