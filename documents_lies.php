...
// Récupérer les documents liés aux véhicules assignés
$stmt = $pdo->prepare("
    SELECT d.id, d.type_document, d.fichier, d.description, v.marque, v.modele
    FROM documents d
    JOIN vehicules v ON d.vehicule_id = v.id
    WHERE d.vehicule_id IN (
        SELECT vehicule_id FROM affectations WHERE utilisateur_id = ? AND date_fin IS NULL
    )
    ...
    <div class="main-content">
        ...
        <div class="content">
            <div class="dashboard-container">
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($documents as $document): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($document['marque'] . ' ' . $document['modele']); ?></td>
                                    <td><?php echo htmlspecialchars($document['type_document']); ?></td>
                                    <td>
                                        <?php if ($document['description']): ?>
                                            <p class="description"><?php echo htmlspecialchars($document['description']); ?></p>
                                        <?php else: ?>
                                            <p class="description">Aucune description disponible</p>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <a href="<?php echo htmlspecialchars($document['fichier']); ?>" class="download" target="_blank">
                ...
                <?php else: ?>
                    <p>Aucun document lié trouvé pour vos véhicules assignés.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
