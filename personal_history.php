...
$stmt = $pdo->prepare("SELECT v.marque, v.modele, v.immatriculation, a.date_debut, a.date_fin 
                      FROM affectations a 
...
// Fetch anomalies
$stmt = $pdo->prepare("SELECT v.marque, v.modele, v.immatriculation, a.commentaire, a.date_signalement 
                      FROM anomalies a 
                      JOIN vehicules v ON a.vehicule_id = v.id 
                      WHERE a.utilisateur_id = :user_id 
...
// Fetch trip logs
$stmt = $pdo->prepare("SELECT v.marque, v.modele, v.immatriculation, s.kilometrage, s.date_enregistrement 
                      FROM suivi_kilometrage s 
                      JOIN vehicules v ON s.vehicule_id = v.id 
                      JOIN affectations a ON s.vehicule_id = a.vehicule_id 
                      WHERE a.utilisateur_id = :user_id 
                      ORDER BY s.date_enregistrement DESC");
$stmt->execute(['user_id' => $_SESSION['user_id']]);
...
<head>
    ...
    <style>
        ...
        .history-container { background: rgba(255, 255, 255, 0.9); padding: 30px; border-radius: 20px; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.3); max-width: 1000px; margin: 20px auto; border: 3px solid #dc3545; color: #333; }
        h2 { color: #dc3545; font-size: 28px; margin-bottom: 20px; }
        h3 { color: #dc3545; font-size: 22px; margin-top: 30px; }
        ...
        @media (max-width: 768px) {
            ...
            .history-container { padding: 20px; margin: 10px; }
            h2 { font-size: 24px; }
            h3 { font-size: 20px; }
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($assignments as $assignment): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($assignment['marque'] . ' ' . $assignment['modele'] . ' (' . $assignment['immatriculation'] . ')'); ?></td>
                                <td><?php echo date('d/m/Y', strtotime($assignment['date_debut'])); ?></td>
                                <td><?php echo $assignment['date_fin'] ? date('d/m/Y', strtotime($assignment['date_fin'])) : 'En cours'; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>

                <h3>Anomalies Signalées</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Véhicule</th>
                    ...
                    <tbody>
                        <?php foreach ($anomalies as $anomaly): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($anomaly['marque'] . ' ' . $anomaly['modele'] . ' (' . $anomaly['immatriculation'] . ')'); ?></td>
                                <td><?php echo htmlspecialchars($anomaly['commentaire']); ?></td>
                                <td><?php echo date('d/m/Y H:i', strtotime($anomaly['date_signalement'])); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($trips as $trip): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($trip['marque'] . ' ' . $trip['modele'] . ' (' . $trip['immatriculation'] . ')'); ?></td>
                                <td><?php echo $trip['kilometrage']; ?> km</td>
                                <td><?php echo date('d/m/Y H:i', strtotime($trip['date_enregistrement'])); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
