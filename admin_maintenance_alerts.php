...
if (isset($_POST['mark_as_handled'])) {
    try {
        ...
        $stmt->execute([$id]);
        header('Location: admin_maintenance_alerts.php?success=Alerte marquée comme traitée');
        exit;
...
try {
    $stmt = $pdo->query("SELECT p.*, v.immatriculation FROM predictions_maintenance p JOIN vehicules v ON p.vehicule_id = v.id WHERE p.date_prevue <= NOW() ORDER BY p.date_prevue DESC");
