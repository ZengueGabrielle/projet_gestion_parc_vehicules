...
$stmt = $pdo->prepare("
    SELECT sk.id, v.marque, v.modele, sk.kilometrage, sk.date_enregistrement
    FROM suivi_kilometrage sk
    JOIN vehicules v ON sk.vehicule_id = v.id
    WHERE sk.vehicule_id IN (
        SELECT vehicule_id FROM affectations WHERE utilisateur_id = ?
    )
    ORDER BY sk.date_enregistrement DESC
");
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($trajets as $trajet): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($trajet['marque'] . ' ' . $trajet['modele']); ?></td>
                                    <td><?php echo htmlspecialchars($trajet['kilometrage']); ?> km</td>
                                    <td><?php echo date('d/m/Y H:i', strtotime($trajet['date_enregistrement'])); ?></td>
                                </tr>
                            <?php endforeach;
                            ?>
                        </tbody>
